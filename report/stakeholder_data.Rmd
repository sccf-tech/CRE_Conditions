...
q.dat.melt=melt(q.dat,id.vars = "Date")
q.dat.melt$value[q.dat.melt$value<0]<-NA

q.dat=cast(q.dat.melt,Date~variable,value="value",mean)
...
meanQ.14=cbind(data.frame(Statistic="Average"),data.frame(t(apply(q.dat[,2:15],2,mean,na.rm=T))))
colnames(meanQ.14)<-c("Statistic",vars[2:length(vars)])

totalQ.14=cbind(data.frame(Statistic="Total"),data.frame(t(apply(q.dat[,2:15],2,sum,na.rm=T))))
