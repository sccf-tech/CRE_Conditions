...
S308q.pos=sum(laketotalQ[laketotalQ$S308>0,c("S308")])
S308q.per=ifelse(S308q<0,NA,round((S308q/q.from.Lake)*100,0))

S310q=sum(laketotalQ[,c("S310")])
S310q.pos=sum(laketotalQ[laketotalQ$S310>0,c("S310")])
...
EAAq.pos=sum(laketotalQ[laketotalQ$EAA>0,c("EAA")])
EAAq.per=ifelse(EAAq<0,NA,round((EAAq/q.from.Lake)*100,0))


laketotalQ2=q.dat.7day[,c("S77","S310","S351","S352","S354","L8","S308")]
tmp=apply(laketotalQ2,2,min)
v=names(tmp[tmp<0])
v[v=="L8"]="C10A"
v.val=knitr::combine_words(v)

q.BF.Lake=sum(abs(laketotalQ2[laketotalQ2<0]))
q.in.Lake=sum(q.dat.7day[,"NthLake"])
```
