...
webpage=read_html("LOK_scrapedpage.html")
...
webpage=read_html("struct_scrapedpage.html")
node.val=html_nodes(webpage,"pre")
...
for(i in 1:(length(dates)-1)){
  ...
  mapdata=readLines("map_dat.html")
...
dates2=c(date.fun(min(Q.dat.xtab$Date)-duration(150,'days')),date.fun(min(Q.dat.xtab$Date)-duration(1,'days')))
...
```{r,echo=FALSE,fig.width=8,fig.height=3.75,fig.align='center'}
...
axis_fun(1,xmaj,xmin,format(xmaj,"%m/%d/%Y"),line=-0.5)
axis_fun(2,ymaj,ymin,ymaj,cex.axis=1)
axis_fun(4,ymaj,ymin,round(cfs.to.acftd(ymaj)/1000,1),cex.axis=1)
...
response <- content(request, as = "text", encoding = "UTF-8")
# if(strsplit(response,"<title>|</title>")[[1]][2]=="503 Service Unavailable"){
#  results=NA;
#  kbrdat=NA
#  }else{
results <- jsonlite::fromJSON(response,flatten=T)
...
```{r CharHar,echo=FALSE,fig.width=5,fig.height=6,fig.align='center'}
...
for(i in 1:5){
  ...
  ylim.max=if(max.q<150){round(max.q+max.q*0.25,-1)}else{round(max.q+max.q*0.25,-2)}
