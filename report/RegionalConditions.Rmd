...
Of the `r nrow(bgdat_30d)` samples collected, cyanobacteria was dominate in `r sum(bgdat_30d$cyanobacteriadominant=="Yes")` samples, `r sum(bgdat_30d$toxinpresent=="Yes")` samples had toxins present ranging from `r min(as.numeric(bgdat_30d$microcystin.val),na.rm = T)` to `r max(as.numeric(bgdat_30d$microcystin.halfmdl),na.rm = T)` `r paste0("\u03BCg L\u207B\u00B9")` (does not include non-detects). 
...
tm_shape(kbrdat_30d.shp)+tm_dots(col="Abundance",
  ...
  tm_shape(HABext.poly)+tm_borders(group="Area of Interest",lwd=1,lty="dashed",col="grey")
```
