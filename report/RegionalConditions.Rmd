...
# 
# check.package.git <- function(repo,pkg){
#   new.pkg <- pkg[!(pkg %in% installed.packages()[, "Package"])]
#   if (length(new.pkg))
...
Estuary had a 7-day average of **`r format(Q7d.sum.tmp[1],big.mark=",")`
cfs** at **S-79** with a 7-day average of
**`r format(Q7d.sum.tmp[3],big.mark=",")` cfs
(`r round((Q7d.sum.tmp[3]/Q7d.sum.tmp[1])*100)`%)** coming from the lake
at **S-77**. **The 14-day moving average flow at S-79 is
`r format(round(salenv.vals$Q.14),big.mark=",")` cfs and has been in the
`r paste0("<span style='color:",cols[which(salenv.vals1==max(salenv.vals1))],"'>")` `r names(salenv.vals1)[which(salenv.vals1==max(salenv.vals1))]` **</span>
flow envelope (`r flow.envs[which(salenv.vals1==max(salenv.vals1))]`
cfs; RECOVER 2020) for
...
**`r format(round(S310q.pos,0),big.mark=",")` AF** through **S-310** in
Clewiston, and **`r format(round(EAAq.pos,0),big.mark=",")` AF** to the
EAA through **S-351**, **S-352**, and **S-354**. The total net inflow to
the Lake was **`r format(round(q.in.Lake+q.BF.Lake,0),big.mark=",")`
AF** (`r format(round(q.in.Lake,0),big.mark=",")` AF from Fisheating
Creek, S-71, S-72, S-84s, S-65EX, and S-65EX1)
`r if(q.BF.Lake>0){paste(" with a total backflow volume of ","**",format(round(q.BF.Lake,0),big.mark=",")," AF** from **", v.val,"**",sep="")}`.
Water conservation areas received flows of
**`r format(round(sum(totalQ[,c("WCA1")]),0),big.mark=",")` AF**,
**`r format(round(sum(totalQ[,c("WCA2")]),0),big.mark=",")` AF**, and
**`r format(round(sum(totalQ[,c("WCA3")]),0),big.mark=",")` AF** at
**WCA1, WCA2, and WCA3**, respectively. Everglades National Park
received **`r format(round(sum(totalQ[,c("ENP")]),0),big.mark=",")`
AF**.
...
`r paste0(strsplit(LO.report[[1]][11],"\\s+")[[1]][5]," ft"," (",strsplit(LO.report[[1]][13],"  Currently in ")[[1]][2],")")`**

**Lake Okeechobee Inflow:
`r paste(LakeIn,"cfs")# paste(strsplit(LO.report[[1]][45],"\\s+")[[1]][3],"cfs")`**
:::

::: col-md-6
**Last Week:
`r paste(strsplit(LO.report[[1]][200],"\\s+")[[1]][9],"ft")`**

**Lake Okeechobee Outflow:
`r paste(LakeOut,"cfs")# paste(strsplit(LO.report[[1]][52],"\\s+")[[1]][3],"cfs")`**
:::
:::
...
::: col-md-3
WP Franklin
**`r with(S79.dat,ifelse(sum(is.na(Man.Prec))!=0,paste("\u2265",format(sum(Man.Prec,na.rm=T),nsmall=2)),format(sum(Man.Prec,na.rm=T),nsmall=2)))`"**
:::

::: col-md-3
Ortona
**`r with(S78.dat,ifelse(sum(is.na(Man.Prec))!=0,paste("\u2265",format(sum(Man.Prec,na.rm=T),nsmall=2)),format(sum(Man.Prec,na.rm=T),nsmall=2)))`"**
:::

::: col-md-3
Moore Haven
**`r with(S77.dat,ifelse(sum(is.na(Man.Prec))!=0,paste("\u2265",format(sum(Man.Prec,na.rm=T),nsmall=2)),format(sum(Man.Prec,na.rm=T),nsmall=2)))`"**
:::
:::
...
<center>Lake Okeechobee daily average stage elevation for water year
`r CurWY` (`r paste0("WY",CurWY)`) relative to the last two water years
(<font color="red">red</font> line) relative to the Lake Okeechobee
Regulation Schedule (LORS) .</center>
...
-   As of
    `r if(is.na(subset(LakeO.xtab, DATE==YEST)$Mean)==T){format(YEST-ddays(1),"%m/%d/%Y")}else{format(YEST,"%m/%d/%Y")}`
    the mean stage elevation in Lake Okeechobee is
    `r if(is.na(subset(LakeO.xtab, DATE==YEST)$Mean)==T){subset(LakeO.xtab,DATE==YEST-ddays(1))$Status}else{subset(LakeO.xtab,DATE==YEST)$Status}`
    Management Band at a stage elevation of
    `r if(is.na(subset(LakeO.xtab, DATE==YEST)$Mean)==T){round(subset(LakeO.xtab,DATE==YEST-ddays(1))$Mean,2)}else{round(subset(LakeO.xtab,DATE==YEST)$Mean,2)}`
    feet (NGVD29).

```{r LOK_recess1,echo=FALSE,fig.width=7,fig.height=5,fig.align='center'}
lwd.val=1
ylim.val=c(-2,3);by.y=1
...
<center>Lake Okeechobee 7-day and 30-day water level recession/ascension
rates during `r paste0("WY",CurWY)`. Negative values indicate falling
water levels (recession) and positive values indicate raising water
level (ascension) during this 7-day period .</center>
...
ylim.val=c(CurWY-3,CurWY);by.y=1;ymaj=seq(ylim.val[1],ylim.val[2],by.y);ymin=seq(ylim.val[1],ylim.val[2],by.y/by.y)
ylim.val2=ylim.val
...
<center>Lake Okeechobee 7-day water level recession/ascension rates
stop-light metric relative to the draft Snail Kite Performance
Measure.</center>
...
rf.dat$cumRF=with(rf.dat,ave(Data.Value,paste(SITE,WY),FUN=function(x) cumsum(ifelse(is.na(x),0,x))))
...
<center>Sonde monitoring locations within Lake Okeechobee conducted by
SFWMD.</center>
...
<center>Daily Chlorophyll, Phycocyanin,relative ratio of Phycocyanin
(Phyco) relative to Phyco and Chlorophyll (Chl) and Turbidity within
Lake Okeechobee monitored by SFWMD. Chlorophyll and Phycocyanin are
plotted on log-scale. Thresholds of concern/risk identified for
Chlorophyll (20 and 40 μg/L) and Phycocyanin (40 μg/L; Cotterill et al
2019 & Macário et al 2015).</center>
...
<li>Cotterill et al (2019) Phycocyanin sensors as an early warning
system for cyanobacteria blooms concentrations: a case study in the
Rotorua lakes. New Zealand Journal of Marine and Freshwater Research
53:555--570.</li>

<li>Macário et al (2015) New insights towards the establishment of
phycocyanin concentration thresholds considering species-specific
variability of bloom-forming cyanobacteria. Hydrobiologia
757:155--165.</li>
...
```{r noaa hab data,echo=F,include=F,eval=F}
...
tmp.raster=raster::raster(paste(data.path, fnames[i],sep="/"))
...
```{r NOAA RS Plot,fig.width=6.5,fig.height=5,fig.align='center',fig.cap="Cyanobacteria Algal Index across Lake Okeechobee. Percent area based on open water coverage (Lake Area minus Littoral Zone). Data from NOAA NCCOS HAB data explorer.Data are provisional and subject to change.",eval=FALSE,include=FALSE}
...
```{r NOAA LOK area,fig.width=6.5,fig.height=5,fig.align='center',fig.cap="Daily cloud cover and cyanobacteria algal bloom coverage of Lake Okeechobee. Percent area based on open water coverage (Lake Area minus Littoral Zone). Data from NOAA NCCOS HAB data explorer.Data are provisional and subject to change. ",eval=FALSE,include=FALSE}
par(family="serif",mar=c(1,3,0.5,2),oma=c(2,2,0.5,2))
layout(matrix(1:2,2,1),heights=c(0.5,1))

xlim.val=c(date.fun("2023-01-01"),date.fun(Sys.Date()+lubridate::duration(1,"months")));xmaj=seq(xlim.val[1],xlim.val[2],"1 months");xmin=seq(xlim.val[1],xlim.val[2],"1 days")
...
```{r noaa hab image,echo=F,include=T}
...
```{r, out.width="100%",echo=FALSE,fig.align="center",fig.cap=paste("Cyanobacteria Index for Lake Okeechobee for",noaa.HAB.image$date,"(Data Source: NOAA NCCOS)"),include=TRUE}

knitr::include_graphics(noquote(gsub("'",'',noaa.HAB.image$fileadd)))
```

```{r, out.width="50%",echo=FALSE,fig.align="center",include=TRUE}
...
# 
# # Blue-Green Algae --------------------------------------------------------
# sdate.algae=date.fun(TODAY-lubridate::duration(15,"days"))
...
#                                               data=bgdat,
#                                               proj4string = wgs84)
# # bgdat.ext=as(extent(bgdat.shp),"SpatialPolygons")
...
```{r BG_map30d,fig.width=6.5,fig.height=4,fig.align='center'}
bgdat=NULL
if(is.null(bgdat)==F){bgdat_30d.shp.notox=subset(bgdat_30d.shp,toupper(toxinpresent)=="NO")}
if(is.null(bgdat)==F){bgdat_30d.shp.tox=subset(bgdat_30d.shp,toupper(toxinpresent)=="YES")}
...
if(is.null(bgdat)==T){bbox.lims=bbox(shore)}else{bbox.lims=bbox(bgdat_30d.shp)}

if(is.null(bgdat)==T){
  plot(shore,col="cornsilk",border="grey",bg="lightblue",ylim=bbox.lims[c(2,4)],xlim=bbox.lims[c(1,3)],lwd=0.01)
  mtext(side=3,line=-10,"No data available\nat this time\n(data server issues)")
  # text(gCentroid(bbox.lims),"No data available\nat this time\n(data server issues)",col="red",cex=3)
  plot(0:1,0:1,ann=F,axes=F,type="n")
  }else{

plot(shore,col="cornsilk",border="grey",bg="lightblue",ylim=bbox.lims[c(2,4)],xlim=bbox.lims[c(1,3)],lwd=0.01)
# if(is.null(bgdat_30d.shp)==F){
plot(subset(bgdat_30d.shp,is.na(toxinpresent)==T),add=T,pch=21,bg="grey",cex=1)
plot(bgdat_30d.shp.notox,add=T,pch=21,bg="green",cex=1,lwd=0.1)
plot(bgdat_30d.shp.tox,add=T,pch=21,bg="red",
     cex=ifelse(bgdat_30d.shp.tox$microcystin.val>8,2,1),lwd=0.1)
 #}
box(lwd=1)
...
legend("center",legend=c("Toxin Present","Microcystin Present >8 \u03BCg L\u207B\u00B9","Toxin Not Present","No Data"),
       pch=c(21),lty=c(NA),lwd=c(0.1),
       col=c("black"),pt.bg=c("red","red","green","grey"),
       pt.cex=c(1,2,1,1),ncol=1,cex=0.8,bty="n",y.intersp=1,x.intersp=0.75,xpd=NA,xjust=0.5,yjust=1,
...
# Of the `r nrow(bgdat_30d)` samples collected, cyanobacteria was dominate
# in `r sum(bgdat_30d$cyanobacteriadominant=="Yes",na.rm=T)` samples,
# `r sum(bgdat_30d$toxinpresent=="Yes",na.rm=T)` samples had toxins
# present ranging from
# `r min(as.numeric(bgdat_30d$microcystin.val),na.rm = T)` to
# `r max(as.numeric(bgdat_30d$microcystin.val),na.rm = T)`
# `r paste0("\u03BCg L\u207B\u00B9")` (does not include non-detects).
...
<center>Blue-green Algae sampling results for the last 14-days for the
10 county area (Lee, Collier, Hendry, Glades, Okeechobee, Martin, Palm
Beach, Broward, Miami-Dade and Monroe). Data from FDEP Blue-green algae
dashboard</center>
...
<center>Fort Myers Yacht Basin salinity (SCCF-RECON) and S79 discharge
(SFWMD) data for the last six months.</center>
...
<center>Fort Myers Yacht Basin salinity (SCCF-RECON) and S79 discharge
(SFWMD) data for the last last 30 days.</center>
...
<center>S-77 (from Lake Okeechobee) and S-79 (to Tidal Basin) flow data
along the C-43 and Caloosahatchee River. C-43 Basin Flow was calculated
as the difference from S-77 and S-79.</center>
...
<center>Daily, 14-day moving average and 30-day moving average discharge
for S-79 relative to the Northern Estuaries RECOVER performance measure
and the established minimium flows and levels (MFLs) for the recent
6-month period.</center>
...
-   As of `r format(YEST,"%b %d")`, discharges conditions are within the
    `r subset(cal.flow.dat.xtab,Date==YEST)$SalEnv.cat` RECOVER salinity
    envelope.
...
<center>Cumulative discharge for S-79 (to Tidal Basin) and S-77 (from
Lake Okeechobee) for the last three Florida Water Years (May - April).
Day of water year is analogous to day of year with Day 1 = May
1<sup>st</sup>.</center>
...
<!-- <center>Top: Daily dissolved organic matter (fluorescence DOM). Middle: -->
<!-- Daily relative fluorescence Chlorophyll and Phycocyanin observed at the -->
<!-- floating sensor bouy at S-79 monitoring by USGS. Bottom: The relative -->
<!-- ratio of Phycocyanin (Phyco) relative to Phyco and Chlorophyll -->
<!-- (Chl).</center> -->
...
# plot(recon.dat$sal)
# 
# fill=data.frame(expand.grid(SiteID=unique(recon.dat$SiteID),Date=date.fun(seq(Start.Date,End.Date,"1 days"))))
# 
# recon.dat$Date=date.fun(recon.dat$Date.EST)
# recon.dat2=ddply(recon.dat,c("Date","SiteName","SiteID"),summarise,sal.mean=mean(sal,na.rm=T),N.val=N.obs(sal))
# 
# recon.dat2=merge(recon.dat2,fill,c("SiteID","Date"),all.y=T)
# recon.dat2$sal.mean=with(recon.dat2,ifelse(N.val<20,NA,sal.mean))
# recon.dat2$MovingAvg.7d=with(recon.dat2,ave(sal.mean,SiteID,FUN=function(x) roll_meanr(x,n=7)))
...
# if(i<3){
# with(subset(recon.dat2,SiteID==recon.sites[i]),pt_line(Date,sal.mean,1,"grey50",1,22,"grey50",cex=1.5,pt.col="black"))
# with(subset(recon.dat2,SiteID==recon.sites[i]),lines(Date,MovingAvg.7d,lty=2,lwd=1.5,col="orange"))
...
# with(subset(recon.dat2,SiteID==recon.sites[4]),pt_line(Date,sal.mean,1,"grey50",2,22,"grey50",cex=1,pt.col="black"))
# with(subset(recon.dat2,SiteID==recon.sites[4]),lines(Date,MovingAvg.7d,lty=2,lwd=1.5,col="orange"))
...
<center>Daily average bottom salinity data for the last 14-days from
sampling locations within the tidal Caloosahatchee River Estuary
relative to oyster health (Sanibel, Shell Point and Cape Coral) and tape
grass (<i>Vallisneria americana</i>) health (Ft. Myers only)
conditions.</center>
...
# 
# months.vals=c(as.Date(TODAY-duration(2,"months")),TODAY)
# request <- GET(
#     url = path,
#     query= list(
...
#                    "AND SAMPLE_DATE >= date",paste0("'",paste0(months.vals[1], " 00:00:00"),"'"),
#                    "AND SAMPLE_DATE < date",paste0("'",paste0(months.vals[2], " 00:00:00"),"'")),
#       outFields = '*',
#       f = 'pjson'
#     )
#   )
...
#   
#   response <- content(request, as = "text", encoding = "UTF-8")
# resp.check=sum(grepl("Unable to complete operation",unlist(strsplit(response,":"))))==1
# if(resp.check==F){
#   results <- jsonlite::fromJSON(response,flatten=T)
#   
#   # results
...
#   
#   
#   kbrdat$cellcount=kbrdat$count_
#   vars=c("date","latitude","longitude","cellcount")
# 
#   kbrdat=kbrdat[,vars]
#   kbrdat$cellcount.log=with(kbrdat,ifelse(cellcount==0,0,log10(cellcount)))
#   kbrdat$category=cat.abund.xwalk$category[findInterval(kbrdat$cellcount.log,c(0,3,4,5,6,10))]
# 
...
kbrdat=NULL
if(is.null(kbrdat)==T){kbrdat_30d.shp=NULL}
```
...
bbox.lims=if(is.null(kbrdat)==T){bbox(shore)}else{bbox(HABext.poly)}
...
plot(0:1,0:1,ann=F,axes=F,type="n")
leg.title=if(is.null(kbrdat_30d.shp$date)==T){
  "Red Tide Abundance"
}else{
  paste0("Red Tide Abundance\n",format(date.rng[1],"%b %d,%Y")," to ",format(date.rng[2],"%b %d,%Y"),"")
}
...
<center>Red tide sampling results for the last 30-days from just south
of Tampa Bay to the Florida Keys. Data from NOAA HABSOS Program</center>
...
Red Tide Prediction and Tracking for coastal Southwest Florida from FWC
and USF
[link](http://ocgweb.marine.usf.edu/hab_tracking/wfcom_hab.html){target="_blank"}.
...
<center>Daily discharge into Charlotte Harbor from major rivers and
creeks. Data source: USGS. Grey dashed lines between points indicate a
potential data gap with values linearly interpolated.</center>
